...
for i in range(5):
    ...
    elastic_client.index(index="tabla2", id=i, document={'numero':float(r)})

data = elastic_client.get(index="tabla2",id=3)
print(str(data['_source']['numero']))
elastic_client.indices.refresh(index="tabla2")
forma = 1
if forma ==0:
    diccionario_bbdd = elastic_client.search(index="tabla2")
    ...
    diccionario_bbdd = elastic_client.search(index="tabla2", filter_path=['hits.hits._source'])
    def media(sum_values,num_values):
        return sum_values/num_values
